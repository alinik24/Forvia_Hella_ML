...
def analyze_and_extract_data(file_path, target_book_state):
    ...
    filtered_output_parquet_path = os.path.join(output_directory,
                                                f"{base_name}_book_state_{target_book_state}_{timestamp}.parquet")
    unique_station_ids_filtered_csv_path = os.path.join(output_directory,
                                                        f"{base_name}_unique_station_ids_filtered_{timestamp}.csv")
    unique_serial_number_ids_filtered_csv_path = os.path.join(output_directory,
                                                              f"{base_name}_unique_serial_number_ids_filtered_{timestamp}.csv")
    ...
    try:
        ...
        print(
            f"Original source file contains {report_data['Unique Station IDs (Original Source File)']} unique station IDs.")
        print(
            f"Original source file contains {report_data['Unique Serial Number IDs (Original Source File)']} unique serial number IDs.")
        ...
        if not df_filtered.empty:
            ...
            print(
                f"Saved {len(df_filtered)} rows with book_state={target_book_state} (from original data) to: {filtered_output_parquet_path}")
            ...
            print(
                f"Filtered original data contains {report_data['Unique Station IDs (Filtered Original Data)']} unique station IDs.")
            print(
                f"Filtered original data contains {report_data['Unique Serial Number IDs (Filtered Original Data)']} unique serial number IDs.")
            ...
            if unique_station_ids_filtered_list:
                pd.DataFrame(unique_station_ids_filtered_list, columns=['station_id']).to_csv(
                    unique_station_ids_filtered_csv_path, index=False)
                print(
                    f"Saved {len(unique_station_ids_filtered_list)} unique Station IDs (from filtered original data) to: {unique_station_ids_filtered_csv_path}")
            else:
            ...
            if unique_serial_number_ids_filtered_list:
                pd.DataFrame(unique_serial_number_ids_filtered_list, columns=['serial_number_id']).to_csv(
                    unique_serial_number_ids_filtered_csv_path, index=False)
                print(
                    f"Saved {len(unique_serial_number_ids_filtered_list)} unique Serial Number IDs (from filtered original data) to: {unique_serial_number_ids_filtered_csv_path}")
            else:
                print("No unique Serial Number IDs found in the filtered original data.")

        else:
            print(
                f"No rows found with book_state={target_book_state} in original data. No filtered Parquet or unique IDs generated.")
        ...
        print(
            f"Error: Missing expected column '{e}' in the Parquet file. Please ensure all required columns are present.")
