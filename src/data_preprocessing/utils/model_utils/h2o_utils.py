...
def save_last_run(data_choice=None, original_raw_path=None, train_path=None, test_path=None, output_path=None, target_column=None, use_validation_frame=None):
    config = {
    ...
    for config_path in config_paths:
        if os.path.exists(config_path):
            try:
                ...
                print(f"⚠️ Could not load configuration from {config_path}: {e}")
    return {}
...
def remove_problematic_columns(train_df, test_df, target_column):
    ...
    if 'station_desc' in train_df.columns:
        ...
        if unique_count <= 1:
            print(f"      ⚠️ station_desc is constant, will be retained as categorical")
        elif missing_ratio > 0.5:
            print(f"      ⚠️ station_desc has high missing rate, filling with 'UNKNOWN'")
...
def comprehensive_schema_harmonization(train_df: pd.DataFrame, test_df: pd.DataFrame, target_column: str) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    print("   🏷️ Processing categorical columns for H2O compatibility...")
    ...
    for col in common_columns:
        ...
        if col == 'measure_value':
            try:
                ...
                print(f"      ⚠️ Failed to process 'measure_value' as numeric: {e}. Treating as categorical.")
    ...
    for col in common_columns:
        ...
        try:
            ...
            if np.isinf(train_harmonized[col]).any() or np.isinf(test_harmonized[col]).any():
                print(f"      ⚠️ Found infinite values in {col}, replacing with median")
                train_harmonized[col] = train_harmonized[col].replace([np.inf, -np.inf], fill_value)
                test_harmonized[col] = test_harmonized[col].replace([np.inf, -np.inf], fill_value)
...
def maximize_automl_configuration(training_frame, validation_frame, x, y, use_validation_frame=True):
    ...
    print(f"⏱️ Total Runtime Budget: {max_runtime_secs} seconds ({max_runtime_secs/60:.0f} minutes)")
...
def train_comprehensive_automl(automl, x, y, training_frame, validation_frame, leaderboard_frame=None):
    print(f"\n🏃‍♂️ TRAINING COMPREHENSIVE AUTOML MODELS")
    print("="*60)
...
def analyze_model_performance(automl, leaderboard, test_frame, y, output_path):
    ...
    try:
            ...
            try:
                ...
                for class_label in sorted(test_actual.unique()):
                    if str(class_label) in class_report:
                        ...
                        print(f"      • Class {class_label}: P={metrics['precision']:.3f}, R={metrics['recall']:.3f}, F1={metrics['f1-score']:.3f}")
            except Exception as auc_error:
                print(f"   ⚠️ AUC or classification report failed: {auc_error}")
        ...
        try:
            ...
            with open(os.path.join(output_path, "model_comparison_metrics.json"), 'w') as f:
                json.dump({
                    ...
                    'best_logloss': float(valid_logloss.min()) if 'logloss' in lb_df.columns and not valid_logloss.empty else None
                }, f, indent=4)
        ...
        return {
            ...
            'performance_summary': {
                'best_logloss': float(valid_logloss.min()) if 'logloss' in lb_df.columns and not valid_logloss.empty else float('nan'),
                'mean_logloss': float(valid_logloss.mean()) if 'logloss' in lb_df.columns and not valid_logloss.empty else float('nan'),
                'best_auc': float(valid_aucs.max()) if 'auc' in lb_df.columns and not valid_aucs.empty else float('nan'),
                'mean_auc': float(valid_aucs.mean()) if 'auc' in lb_df.columns and not valid_aucs.empty else float('nan')
            },
            'test_metrics': {
...
def create_comprehensive_visualizations(analysis_results, output_path):
    ...
    try:
        ...
        if lb_df.empty:
            print("   ⚠️ No leaderboard data available for visualization")
            return
        ...
        if 'test_actual' in analysis_results and 'test_predictions' in analysis_results:
            ...
            if not test_actual.empty and not test_predictions.empty and 'predict' in test_predictions.columns:
                try:
                    ...
                    print(f"   ⚠️ Could not create confusion matrix: {cm_error}")
    ...
    try:
        ...
        if perf_summary:
            summary['model_performance_distribution'] = {
                'best_logloss': round(perf_summary.get('best_logloss'), 6) if not pd.isna(perf_summary.get('best_logloss', float('nan'))) else None,
                'mean_logloss': round(perf_summary.get('mean_logloss'), 6) if not pd.isna(perf_summary.get('mean_logloss', float('nan'))) else None,
                'best_auc': round(perf_summary.get('best_auc'), 6) if not pd.isna(perf_summary.get('best_auc', float('nan'))) else None,
                'mean_auc': round(perf_summary.get('mean_auc'), 6) if not pd.isna(perf_summary.get('mean_auc', float('nan'))) else None
            }
...
def run_comprehensive_automl_pipeline(train_path, test_path, output_path, target_column='target', use_validation_frame=True):
    try:
        print("\n" + "="*70)
        ...
        summary_stats = {
            'best_algorithm': analysis_results['champion_model'].__class__.__name__ if analysis_results['champion_model'] else 'None',
