...
from src.data_preprocessing.config import INPUT_FILE_BOOKINGS_SL, OUTPUT_FILE_BOOKINGS_SL_enc, VECTORIZER_PATH_BOOKINGS
...
def main():
    print("Loading data...")
    try:
        df = load_parquet_dataset(INPUT_FILE_BOOKINGS_SL)
    ...
    try:
        ...
        y, feature_names, preprocessed_df = preprocess_data(
            ...
            vectorizer_path=VECTORIZER_PATH_BOOKINGS,
    ...
    try:
        ...
        preprocessed_df.to_parquet(OUTPUT_FILE_BOOKINGS_SL_enc, index=False, engine='pyarrow')
        print(f"Successfully saved preprocessed data to {OUTPUT_FILE_BOOKINGS_SL_enc}")
        ...
        try:
            ...
            preprocessed_df.to_csv(OUTPUT_FILE_BOOKINGS_SL_enc.replace('.parquet', '.csv'), index=False)
            print(f"Saved as CSV: {OUTPUT_FILE_BOOKINGS_SL_enc.replace('.parquet', '.csv')}")
