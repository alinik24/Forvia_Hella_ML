...
def main():
    ...
    if reuse_last:
        ...
        if not input_path:
            ...
            input_path = input("Enter path to encoded materials dataset (parquet): ").strip()
    else:
        input_path = input("Enter path to encoded materials dataset (parquet): ").strip()

    print(f"Loading materials dataset from {input_path} ...")
    try:
    ...
    print("Running Ridge regression on materials dataset...")
