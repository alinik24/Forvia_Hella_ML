...
from src.data_preprocessing.config import VECTORIZER_PATH_MEAS, OUTPUT_FILE_MEAS_SL_enc, INPUT_FILE_MEAS_SL
from src.data_preprocessing.utils.preprocessing_utils import preprocess_data, load_parquet_dataset


def main():
    print("Loading data...")
    try:
        df = load_parquet_dataset(INPUT_FILE_MEAS_SL)
    ...
    try:
        ...
        y, feature_names, preprocessed_df = preprocess_data(
            ...
            boolean_cols=[],
            vectorizer_path=VECTORIZER_PATH_MEAS,
    ...
    try:
        ...
        preprocessed_df.to_parquet(OUTPUT_FILE_MEAS_SL_enc, index=False, engine='pyarrow')
        print(f"Successfully saved preprocessed data to {OUTPUT_FILE_MEAS_SL_enc}")
        ...
        try:
            ...
            preprocessed_df.to_csv(OUTPUT_FILE_MEAS_SL_enc.replace('.parquet', '.csv'), index=False)
            print(f"Saved as CSV: {OUTPUT_FILE_MEAS_SL_enc.replace('.parquet', '.csv')}")
